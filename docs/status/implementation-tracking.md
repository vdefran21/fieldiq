...
| ✅ | Bug 5: Counter slots discarded by inbound handler | `handleIncomingResponse()` expanded: creates proposal record for counter, intersects counter slots with local availability, transitions to `pending_approval` on match or `failed` at max rounds. | `NegotiationService.kt` — counter path in `handleIncomingResponse()`. 4 new unit tests: `counterCreatesProposal`, `counterMatchTransitions`, `counterAtMaxRoundsFails`, `acceptedNoCounter`. Session 46 extends the same inbound counter handler so a counter received while the shadow session is already in `pending_approval` clears the agreed slot and confirmation flags, reopens the session to `proposing`, and republishes the transition for realtime clients. Session 47 complements that by allowing `respondToProposal()` to revise a matched slot even when the current manager originally proposed it, which matches the spec's "one side requests different time" rule in `pending_approval`. Added unit regressions: `counterFromPendingApprovalReopensProposals`, `counterFromPendingApprovalWithoutCounterpartProposal`. Verification: `cd backend && ./gradlew test` passed. |
...
| ✅ | 06 | `.ics` download link for confirmed games | Backend: `EventController.kt` `/events/{eventId}/ics`, `EventService.buildIcsExport()`, `EventDto.icsUrl`. Mobile: `mobile/services/api.ts`, `mobile/app/(app)/negotiate/[id].tsx` — the app now downloads the protected `.ics` payload with bearer auth and opens/shares the local file, avoiding the prior 401 caused by opening the JWT-protected backend URL directly in Safari. Verification: `cd mobile && npm run lint`, `cd mobile && npx tsc --noEmit` passed. |
...
| Status | Doc | Task | Evidence / Notes |
|--------|-----|------|------------------|
| 🔧 | 00 | Two managers on two physical iOS devices, full negotiation flow | Live validation on phone + simulator now reaches initiate/join/propose/counter/confirm, but Session 48 surfaced a product gap: after a counter returns the session to `proposing`, both devices still show a passive "Finding mutual time" state with no clear guidance that a manager must manually tap `Send proposal round` to continue. This is a UX/usability blocker rather than a protocol failure and should drive the next mobile workflow pass. |
...
| 7 | End-to-End Integration | 🔧 In Progress | 0/5 | 5 |
